...
STARTLPAMOUNT = 100 # 최초 시작시 체력
STARTEPAMOUNT = 100 # 최초 시작시 기력
    ...
    def enter(self, event):
        ...
        if event == EV_LD:
            ...
            # 왼쪽 키를 누르고 있을 때
                ...
                # 시간 안에 다시 같은 키를 누르고 그림 그리는 중이 아니고 대기 시간이 아니라면
                if frame_game.keypressedz == 0 and frame_game.nowdashtime > 0 and frame_game.nowskillxcooltime == 0:
                    frame_game.nowdashtime = 0
                    frame_game.ifnowclickl = 0
                    frame_game.nowdashl = 1  # 왼쪽 도약 시작
...
def animationshow(self):
    if frame_game.nowdashr == 1:  # 오른쪽 도약 실행시
        ...
        frame_game.nowskillmovecooltime = frame_game.skillmovecooltime  # 대기시간 켜기
    ...
    # 대기 시간 (쿨타임) 감소
    if frame_game.nowskillmovecooltime > 0:
        frame_game.nowskillmovecooltime -= 1
        # 대기 시간이 일정 비율 이하로 줄어들면 잔상표시 지우기
        if frame_game.nowskillmovecooltime <= frame_game.skillmovecooltime * (3 / 4):
...
class Wingimage:
    ...
    def draw(self, x, y, drdir):
        ...
        # 날기(점프) 효과 이미지 그리기
        self.image.clip_composite_draw(0, self.frame * 24, 55, 14, rad, flip, frame_game.eunbi.x,
